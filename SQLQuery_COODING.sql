CREATE DATABASE LibraryDBproject;
use LibraryDBproject;
...
SELECT MemberID, FullName FROM MEMBER_TABLE ORDER BY MemberID;
INSERT INTO MEMBER_TABLE (FullName, Email, PhoneNumber, MembershipStartDate)
VALUES ('nasser', 'nasser@hotmail.com', '900-4444', GETDATE());
...
(2, 3, '2025-12-05', '2025-12-20', NULL, 'Issued'),
(4, 1, '2025-04-01', '2025-04-15', NULL, 'Issued'),
(5, 2, '2025-05-03', '2025-05-18', NULL, 'Issued'),
(6, 3, '2025-02-01', '2025-08-13', NULL, 'Issued');

SELECT BookID, Title FROM Book ORDER BY BookID;
...
-- 1. Create / Replace the procedure
...
BEGIN
    ...
    DECLARE @FinePerDay DECIMAL(10,2) = 2.00;
    DECLARE @TotalFine DECIMAL(10,2) = 0;
    ...
    IF @DueDate IS NULL
    BEGIN
        RAISERROR('Error: Loan record not found.', 16, 1);
        RETURN;
    END

    -- Check if already returned
    IF EXISTS (SELECT 1 FROM Loan_table WHERE LoanID = @LoanID AND Status = 'Returned')
    BEGIN
        RAISERROR('Error: Loan has already been returned.', 16, 1);
        RETURN;
    END
    ...
    IF @DaysOverdue > 0
        SET @TotalFine = @DaysOverdue * @FinePerDay;
    ...
    -- Insert payment record if there is a fine
    IF @TotalFine > 0
    BEGIN
        INSERT INTO Payment (LoanID, PaymentDate, Amount, Method)
        VALUES (@LoanID, @ReturnDate, @TotalFine, 'Pending');
    END

    PRINT 'Return processed successfully. Total fine: ' + CAST(@TotalFine AS NVARCHAR(20));
END;
GO
DECLARE @Today DATE = CAST(GETDATE() AS DATE);
EXEC sp_ReturnBook @LoanID = 1, @ReturnDate = @Today;

EXEC sp_ReturnBook @LoanID = 1, @ReturnDate = (CAST(GETDATE() AS DATE));
...
SELECT LoanID, MemberID, BookID, Status
FROM Loan_table
WHERE MemberID = 2;
...
SELECT LoanID, MemberID, BookID, LoanDate, DueDate, Status
FROM Loan_table
WHERE MemberID = 2
